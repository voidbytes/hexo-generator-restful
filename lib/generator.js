...
function minify (str) {
    return hexoUtil.stripHTML(str).trim().replace(/\n/g, ' ').replace(/\s+/g, ' ');
}
    ...
    var restful = cfg.hasOwnProperty('restful') ? cfg.restful :
        ...
        postMap = function (post) {
            return {
                ...
                excerpt: posts_props('excerpt', post.excerpt ?minify(post.excerpt): minify(post.content).substring(0, 50)),
        ...
        apiData = apiData.concat(posts.map(function (post) {
            ...
            return {
                ...
                data: JSON.stringify({
                    ...
                    path: path,
                    excerpt: post.excerpt ?minify(post.excerpt): minify(post.content).substring(0, 50) ,
